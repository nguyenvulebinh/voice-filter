...
from src.net.tcn import TemporalConvNet
from espnet2.enh.separator.abs_separator import AbsSeparator

is_torch_1_9_plus = V(torch.__version__) >= V("1.9.0")


class TCNSeparatorVoiceFilter(AbsSeparator):
    def __init__(
        self,
        input_dim: int,
        output_dim: int = None,
        ...
        if output_dim is not None:
            speaker_emb_dim = input_dim - output_dim
            input_dim = input_dim - speaker_emb_dim
        else:
            speaker_emb_dim = 0
    ...
    def forward(
        ...
        speaker_embed: Union[torch.Tensor, ComplexTensor],
        ilens: torch.Tensor,
        ...
        speaker_embed = speaker_embed.repeat(1, feature.size(1), 1)
        # dvec: [B, L, emb_dim]
        feature = torch.cat((feature, speaker_embed), dim=2)  # [B, L, num_mel + emb_dim]
