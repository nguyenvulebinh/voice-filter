...
class TemporalConvNet(nn.Module):
    def __init__(
        self, N, B, H, P, X, R, C, norm_type="gLN", causal=False, mask_nonlinear="relu", speaker_emb_dim=0,
    ):
        ...
        # Components
        # [M, N, K] -> [M, N, K]
        layer_norm = ChannelwiseLayerNorm(N + speaker_emb_dim)
        # [M, N, K] -> [M, B, K]
        bottleneck_conv1x1 = nn.Conv1d(N + speaker_emb_dim, B, 1, bias=False)
        # [M, B, K] -> [M, B, K]
        repeats = []
        for r in range(R):
            blocks = []
    ...
    def forward(self, mixture_w):
        ...
        score = score.view(M, self.C, self.N, K)  # [M, C*N, K] -> [M, C, N, K]
