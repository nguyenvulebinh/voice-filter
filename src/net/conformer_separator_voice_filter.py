...
class ConformerSeparatorVoiceFilter(AbsSeparator):
    def __init__(
        self,
        input_dim: int,
        output_dim: int = None,
        ...
        self.linear = torch.nn.ModuleList(
            [torch.nn.Linear(adim, output_dim) for _ in range(num_outputs)]
        )
    ...
    def forward(
        ...
        # dvec: [B, T, emb_dim]
        feature = torch.cat((feature, speaker_embed), dim=2)  # [B, T, num_mel + emb_dim]
